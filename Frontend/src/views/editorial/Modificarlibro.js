...
export default function Vistamodificarlibro() {
  ...
  useEffect(() => {
   ...
   async function Obtenerdatoslibro(){
    const rawResponse = await getunLibro(id_libro);
    const respuesta = await rawResponse.json();
    //console.log("RESPUESTA ES "+respuesta.nombre)
    return respuesta;

   }

   Obtenerdatoslibro().then((respuesta) => {
    if (respuesta.status === 200) {
      console.log("LOS DATOS DEL LIBRO SON" +respuesta.data[0].generos[0]);
    
      nombreRef.current.value=respuesta.data[0].nombre
      stockRef.current.value=respuesta.data[0].stock
      autorRef.current.value=respuesta.data[0].autor
      paginasRef.current.value=respuesta.data[0].numeropaginas
       fechapublicacionRef.current.value=respuesta.data[0].fechapublicacion
       idiomaRef.current.value=respuesta.data[0].idioma
       precioRef.current.value=respuesta.data[0].precio
       globlal_idlibro=respuesta.data[0].id_libro
       for(let i=0;i<respuesta.data[0].generos.length;i++){
           console.log(respuesta.data[0].generos[i])
        selected.push({value:respuesta.data[0].generos[i],label:respuesta.data[0].generos[i]})
       }
    
       let preview = document.getElementById("preview"),
       image = document.createElement("img");

     image.src =respuesta.data[0].foto ;
  ...
  async function Modificarlibro(e) {
 ...
 //console.log("EL IDE")
   ...
   if (archivos.files.length == 0) {
    //----llamo a crear album
    subirarchivo = await ModificarLibro(
        id_libro,
        ...
        console.log("VOY A ENVIAR CON FOTO")
        subirarchivo = await ModificarLibro(
            id_libro,
            nombreRef.current.value,
            generos,
      ...
      <div className="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-blueGray-100 border-0">
        <div className="rounded-t bg-white mb-0 px-6 py-6">
          <div className="text-center flex justify-between">
            <h6 className="text-blueGray-700 text-xl font-bold">Modificar Libro</h6>
            <button
